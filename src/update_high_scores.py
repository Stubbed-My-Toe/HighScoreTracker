...
#high scores is a dictionary ={
    with open(path,'r') as file:
        ...
        for row in reader:
            high_scores.append([row[0],int(row[1])])
    return high_scores
...
def show_scores():
    ...
    for i, v in enumerate(get_scores('docs/gtn_scores.csv')):
        print(f'{i}: {v[0]} got {v[1]}')
    print('\nTic tac toe high scores:')
    for i, v in enumerate(get_scores('docs/ttc_scores.csv')):
        print(f'{i}: {v[0]} got {v[1]}')

def add_score(user,score,path):
    high_scores=get_scores(path)
    high_scores.append([user,score])
    high_scores=sorted(high_scores, key=lambda x: x[1])
